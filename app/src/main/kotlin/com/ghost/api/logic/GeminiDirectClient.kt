...
object GeminiDirectClient {
    ...
    suspend fun generateContent(
        ...
        try {
            val endpoint = URL("$BASE_URL?key=${apiKey.trim()}")
            ...
            if (responseCode in 200..299) {
                ...
                if (candidates != null && candidates.size() > 0) {
                    ...
                    Pair(true, finalReply.ifBlank { "Gemini returned an empty response." })
                } else {
                    Pair(false, "Gemini did not return any candidate response.")
                }
            } else {
                ...
                Pair(false, "Gemini API error ($responseCode): ${errorStream.take(200)}")
            }
        } catch (e: Exception) {
            Timber.e(e, "GeminiDirectClient generateContent failed")
            Pair(false, "Network error consulting Gemini: ${e.message}")
        } finally {
